...
public class NewsService {
    ...
    public List<Map<String, Object>> getRecommended(UserPrincipal userPrincipal, Double latitude, Double longitude, int page, int pageSize, List<String> excludeIds) {
        ...
        if (userCountryCode != null) {
            try {
                String url = UriComponentsBuilder.fromHttpUrl(baseUrl + "/top-headlines")
                        .queryParam("country", userCountryCode.toLowerCase())
                        .queryParam("pageSize", pageSize)
                        .queryParam("page", Math.max(1, page))
                        .queryParam("apiKey", apiKey)
                        .toUriString();
                ...
                if (localCandidates == null || localCandidates.isEmpty()) {
                    if (userCountryName != null) {
                        String fallbackUrl = UriComponentsBuilder.fromHttpUrl(baseUrl + "/everything")
    ...
    private List<Map<String, Object>> fetchAndTransform(String url, String category) {
        try {
            log.info("Fetching news from URL: {}", url.replaceAll("apiKey=[^&]+", "apiKey=***"));
            org.springframework.http.HttpHeaders headers = new org.springframework.http.HttpHeaders();
            headers.set("User-Agent", "DailyFeedApp/1.0 (contact: final-year-project-developer@newsapp.com)");
            org.springframework.http.HttpEntity<String> entity = new org.springframework.http.HttpEntity<>(headers);
            ...
            if (response == null || !"ok".equals(response.get("status"))) {
                log.warn("News API response status not OK: {}", response);
                return Collections.emptyList();
            }
            ...
            for (Map<String, Object> art : articles) {
                // Ignore articles with deleted content
                String title = (String) art.get("title");
                if (title == null || title.contains("[Removed]")) {
                    continue;
                }
                ...
                transformed.put("id", articleUrl);
                transformed.put("title", title);
                
                String description = (String) art.get("description");
                String content = (String) art.get("content");
                transformed.put("content", content != null ? content : (description != null ? description : "No content available."));
                
                transformed.put("category", category);
                
                String author = (String) art.get("author");
                transformed.put("author", author != null ? author : "Unknown Author");
                
                String imageUrl = (String) art.get("urlToImage");
                ...
                transformed.put("publishedAt", art.get("publishedAt"));
                
                // Calculate dynamic read time
                int wordsCount = (content != null ? content.split("\\s+").length : 100);
